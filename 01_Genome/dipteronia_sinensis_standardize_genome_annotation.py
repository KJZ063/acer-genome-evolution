"""
用途 | Purpose:
- 标准化 Dipteronia sinensis（Dsin）的基因组注释与序列命名，生成发布版 GFF3、genome、CDS、pep 及 ID 映射表。
- Standardize the annotation and sequence naming for Dipteronia sinensis (Dsin) and generate release-ready GFF3, genome, CDS, peptide files, and ID mapping tables.
输入 | Inputs:
...
file_input_gff3 = resolve_input_path("Dipteronia_genomes", "jinqianqi", "03.structure_annotation", "pasa2.longest.filter.gff3")
...
file_input_genome = resolve_input_path("Dipteronia_genomes", "jinqianqi", "00.Genome", "Dis.genome.fa")
...
file_input_cds = resolve_input_path("Dipteronia_genomes", "jinqianqi", "03.structure_annotation", "pasa2.longest.filter.cds")
...
file_input_pep = resolve_input_path("Dipteronia_genomes", "jinqianqi", "03.structure_annotation", "pasa2.longest.filter.pep")
...
with open(file_input_genome, "r") as fr:
    for line in fr:
        if line.startswith(">"):
            seqid = seqid_replace_dict[line.strip().replace(">", "")]
            genome_dict[seqid] = ""
            print(f"reading {seqid}")
        else:
            genome_dict[seqid] += line.strip()
...
for seqid in sorted(gene_dict.keys()):
    ...
    if "scaffold" in seqid:
        index_seqid = seqid.replace("Dsin_scaffold", "")
...
with open(file_input_cds, "r") as fr:
    for line in fr:
        if line.startswith(">"):
            cds_id = mrna_replace_dict[line.strip().replace(">", "")]
            cds_dict[cds_id] = ""
        else:
...
with open(file_input_pep, "r") as fr:
    for line in fr:
        if line.startswith(">"):
            pep_id = mrna_replace_dict[line.strip().replace(">", "")]
            pep_dict[pep_id] = ""
        else:
