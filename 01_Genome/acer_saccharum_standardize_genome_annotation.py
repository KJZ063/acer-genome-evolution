"""
用途 | Purpose:
- 标准化 Acer saccharum（Asac）的基因组注释与序列命名，生成发布版 GFF3、genome、CDS、pep 及 ID 映射表。
- Standardize the annotation and sequence naming for Acer saccharum (Asac) and generate release-ready GFF3, genome, CDS, peptide files, and ID mapping tables.
输入 | Inputs:
...
file_input_gff3 = resolve_input_path("Asac", "original", "Asac.original.gff3")
...
file_input_genome = resolve_input_path("Asac", "Asac.genome.fa")
...
file_input_cds = resolve_input_path("Asac", "original", "Asac.original.cds.fa")
...
file_input_pep = resolve_input_path("Asac", "original", "Asac.original.pep.fa")
...
with open(file_input_gff3, "r") as fr:
    for line in fr:
        ...
        seqid = seqid_replace_dict[items[0].replace("acsa_", "Contig")]
        type = items[2]
        start_pos = items[3]
