"""
用途 | Purpose:
- 标准化 Acer negundo（Aneg）的基因组注释与序列命名，生成发布版 GFF3、genome、CDS、pep 及 ID 映射表。
- Standardize the annotation and sequence naming for Acer negundo (Aneg) and generate release-ready GFF3, genome, CDS, peptide files, and ID mapping tables.
输入 | Inputs:
...
4. 为原始 GFF3 缺失的 gene 行补齐统一结构。 / Add missing gene rows so the original GFF3 follows a unified structure.
5. 重写 GFF3、CDS 和蛋白序列文件，并输出对应映射表。 / Rewrite GFF3, CDS, and peptide files and export the related mapping tables.
"""
...
file_input_gff3 = resolve_input_path("Aneg", "original", "Aneg.original.gff3")
...
file_input_genome = resolve_input_path("Aneg", "Aneg.genome.fa")
...
file_input_cds = resolve_input_path("Aneg", "original", "Aneg.original.cds.fa")
...
file_input_pep = resolve_input_path("Aneg", "original", "Aneg.original.pep.fa")
...
seqid_replace_dict = {}
seq_length = {}
print(f"Reading {file_input_genome}.")
with open(file_input_genome, "r") as fr:
    ccc = 0
    for line in fr:
        if line.startswith(">"):
            old_seqid = line.strip().replace(">", "")
            seq_length[old_seqid] = 0
        else:
            seq_length[old_seqid] += len(line.strip())
seqid_list = list(dict(sorted(seq_length.items(), key=lambda x: x[1])).keys())[::-1]
fw = open(file_output_seqid_replace, "w")
print(f"Writing seq_id replace list.")
i = 0
for old_seqid in seqid_list:
    i += 1
    if i < 14:
        new_seqid = f"{sp}{str(i).zfill(2)}"
    else:
        new_seqid = f"{sp}_scaffold{old_seqid.replace('Contig', '')}"
    seqid_replace_dict[old_seqid] = new_seqid
    fw.write(f"{new_seqid}\t{old_seqid}\t{seq_length[old_seqid]}\n")
fw.close()
...
# Original GFF3 has only mRNA, exon and intron / 原始 GFF3 仅含 mRNA、exon 和 intron
# Add gene rows for consistency; rename mRNA as gene_id.1 / 为统一格式补齐 gene 行，并将 mRNA 命名为 gene_id.1
...
with open(file_input_gff3, "r") as fr:
    for line in fr:
        count_line += 1
        print(f"Finishing {count_line} lines.", end="\r")
        items = line.strip().split("\t")
        seqid = seqid_replace_dict[items[0].replace("acne_", "Contig")]
        type = items[2]
        start_pos = items[3]
        ...
        if type == "mRNA":
            count_gene += 1
            # print(f"Finishing {count_gene} genes.", end="\r")
...
with open(file_input_gff3, "r") as fr:
    for line in fr:
        ...
        if type == "mRNA":
            ...
            new_gene_id = gene_replace_dict[old_mrna_id]
            new_mrna_id = f"{new_gene_id}.1"
            mrna_replace_dict[old_mrna_id] = new_mrna_id
            write_line = f"{seqid}\tGFACS\tgene\t{tab.join(items[3:-1])}\tID={new_gene_id}\n" \
                         f"{seqid}\t{tab.join(items[1:-1])}\tID={new_mrna_id};Parent={new_gene_id}\n"
        elif type == "exon":
            count_exon += 1
            suffix = f"exon{count_exon}"
            new_id = f"{new_mrna_id}.{suffix}"
            write_line = f"{seqid}\t{tab.join(items[1:-1])}\tID={new_id};Parent={new_mrna_id}\n"
        elif type == "intron":
            count_intron += 1
