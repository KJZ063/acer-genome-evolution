"""
用途 | Purpose:
- 标准化 Acer yangbiense（Ayan）的基因组注释与序列命名，生成发布版 GFF3、genome、CDS、pep 及 ID 映射表。
- Standardize the annotation and sequence naming for Acer yangbiense (Ayan) and generate release-ready GFF3, genome, CDS, peptide files, and ID mapping tables.
输入 | Inputs:
...
file_input_gff3 = resolve_input_path("Ayan", "original", "Ayan.original.gff3")
...
file_input_genome = resolve_input_path("Ayan", "original", "Ayan.original.genome.fa")
...
file_input_cds = resolve_input_path("Ayan", "original", "Ayan.original.cds.fa")
...
file_input_pep = resolve_input_path("Ayan", "original", "Ayan.original.pep.fa")
...
for seqid in sorted(seqid_list):
    if "CTG" in seqid.upper():
        index_scaffold = seqid.replace("ctg", "")
    ...
    elif "CHR" in seqid.upper():
        index_seqid = seqid.replace("chr", "")
        fw.write(f"{sp}{str(index_seqid).zfill(2)}\t{seqid}\n")
        seqid_replace_dict[seqid] = f"{sp}{str(index_seqid).zfill(2)}"
    else:
        fw.write(f"{sp}{seqid}\t{seqid}\n")
        seqid_replace_dict[seqid] = f"{sp}{seqid}"
fw.close()
