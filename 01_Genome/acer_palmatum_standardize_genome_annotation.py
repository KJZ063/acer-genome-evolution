"""
用途 | Purpose:
- 标准化 Acer palmatum（Apal）的基因组注释与序列命名，生成发布版 GFF3、genome、CDS、pep 及 ID 映射表。
- Standardize the annotation and sequence naming for Acer palmatum (Apal) and generate release-ready GFF3, genome, CDS, peptide files, and ID mapping tables.
输入 | Inputs:
...
file_input_gff3 = resolve_input_path("Apal", "original", "JZ.final.longest.gff3")
...
file_input_genome = resolve_input_path("Apal", "original", "jz.LG.fasta")
...
file_input_cds = resolve_input_path("Apal", "original", "JZ.cds.fa")
...
file_input_pep = resolve_input_path("Apal", "original", "JZ.pep.fa")
...
for seqid in sorted(seqid_list):
    if "CONTIG" in seqid.upper():
        index_scaffold = seqid.replace("Contig", "")
