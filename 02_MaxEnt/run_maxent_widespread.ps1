<#
用途 | Purpose:
- 批量运行广布种的未来情景 MaxEnt 投影。
- Batch-run the future-scenario MaxEnt projections for widespread species.
输入 | Inputs:
...
$scenario = "wc2.1_30s_bioc_BCC-CSM2-MR_ssp585_2081-2100"

# 运行物种配置 / Species configuration for this batch
$runs = @(
    @{ Species = "Atru"; MaxBackground = 100000 },
    @{ Species = "Ddye"; MaxBackground = 100000 },
    @{ Species = "Asac"; MaxBackground = 1000000 }
)

foreach ($run in $runs) {
    $outDir = Join-Path $outputRoot ("{0}_{1}_output" -f $run.Species, $scenario)
    if (!(Test-Path $outDir)) {
        New-Item -ItemType Directory -Path $outDir -Force | Out-Null
    }

    $projectionLayers = Join-Path $projectionRoot $scenario
    ...
    $arguments = @(
        ...
        "maximumbackground=$($run.MaxBackground)",
