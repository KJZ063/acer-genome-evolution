# 用途 | Purpose:
# - 运行 Acer saccharum 的单物种 MaxEnt 分析。
# - Run the single-species MaxEnt analysis for Acer saccharum.
# 输入 | Inputs:
...
# 输出 | Outputs:
# - 在 Asac_300kmBuffer 下输出背景点、occurrence 表、MaxEnt 评估结果和图件。
# - Write background points, occurrence tables, MaxEnt evaluation results, and figures under Asac_300kmBuffer.
# 使用提示 | Usage Notes:
...
run_single_species_maxent(get_single_species_config(
  ...
  buffer_km = 300,
  background_n = 200000,
  background_sample_n = 100000,
  output_subdir = "Asac_300kmBuffer"
))
